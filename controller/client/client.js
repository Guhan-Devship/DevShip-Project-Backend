...
module.exports = (app, io) => {
      ...
      if (user !== undefined && user !== null) {
        library.validPassword(req.body.password, user.password, (error, isValidPassword) => {
          if (isValidPassword) {
            ...
            data.status = 0;
            data.response = { message: 'Invalid Password' };
            res.json(data);
          }
        });
      } else {
  ...
  router.registration = async (req, res) => {
    ...
    let getUserEmail = await GetOneDocument('client', { email: req.body.email }, {}, {});
    ...
    if (insert && insert._id) {
      ...
      res.json({ status: 0, message: 'Failed to create user' });
    }
  };
